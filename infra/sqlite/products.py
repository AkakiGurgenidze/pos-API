    ...
    def create(self, product: Product) -> None:
        try:
            self.cur.executemany(
                "insert into products(id, unit_id, name, barcode, price) values (?,?,?,?,?)",
                [(str(product.id), str(product.unit_id), product.name, product.barcode, product.price)],
            )
            self.con.commit()
            ...
            elif "UNIQUE constraint failed" in error_message and "barcode" in error_message:
